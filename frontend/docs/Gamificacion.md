...
Se agregaron los campos `level`, `experience`, `nextLevelExperience` y `culturalAchievements` a la entidad `Gamification` en `backend/src/features/gamification/entities/gamification.entity.ts`.
Se reemplazó la entidad `Level` con `UserLevel` en `backend/src/features/gamification/entities/user-level.entity.ts`.
