...
const HomePage = () => {
  ...
  const featureCardCommonClasses = 'hover:scale-105 transition-transform duration-300 hover:bg-gray-100';
  ...
  return (
    ...
    >
      ...
      <Carousel className="w-full max-w-5xl mx-auto" role="region" aria-roledescription="carousel">
        <CarouselContent className="w-full h-[500px]" role="group" aria-label="Elementos del carrusel"> {/* Añadida altura fija */}
          {heroCardsData.map((card, index) => (
            <CarouselItem key={index} className="md:w-1/2 lg:w-1/3 h-full"> {/* Asegurar que el item ocupe la altura */}
              <HeroSection
                title={card.title}
                ...
                imageAlt={card.alt}
                onComienzaAhoraClick={handleComienzaAhoraClick}
              />
            </CarouselItem>
          ))}
        </CarouselContent>
      ...
      {/* Features Section */}
          <section className="py-12"> {/* Ajustado espaciado */}
      ...
      <section className="py-12"> {/* Ajustado espaciado */}
        ...
        >
          ...
          >
            <CarouselContent role="group" aria-label="Elementos del carrusel de testimonios"> {/* Añadir rol y etiqueta ARIA */}
              {testimonials.map((testimonial, index) => (
                <CarouselItem key={index}>
                  <Card className=""> {/* Eliminado borde amarillo */}
          ...
          {testimonialCarouselApi && (
            ...
            >
              {isTestimonialAutoplayPlaying ? <Pause className="size-4" /> : <Play className="size-4" />}
            </Button>
          )}
        </motion.div>
      </section>
      ...
      {/* Contact Section */}
      <section className="py-12 bg-white"> {/* Ajustado espaciado */}
        <h2 className="text-2xl font-bold text-center mb-4">¿Tienes preguntas?</h2>
        <ContactForm />
      </section>

      {/* Footer */}
      <footer className="text-center py-4 text-gray-500 text-sm"> {/* Increased padding */}
        <div className="flex justify-center space-x-4 mb-2">
          <HashLink to="/privacy" smooth={true} duration={500} className="hover:underline">Política de Privacidad</HashLink>
          <HashLink to="/terms" smooth={true} duration={500} className="hover:underline">Términos de Servicio</HashLink>
          <HashLink to="/contact" smooth={true} duration={500} className="hover:underline">Contacto</HashLink>
        </div>
